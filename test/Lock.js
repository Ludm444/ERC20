...
const tocenJSON = require("../artifacts/contracts/ERC.sol/MSC.json")
describe("MyShop", function() {
  ...
  beforeEach(async function () {
    ...
    await shop.deployed()
    erc20 = new ethers.Contract(await shop.token(), tocenJSON.abi, owner)
  })

  it("shoul be token and owner", async function() {
    expect(await shop.owner()).to.eq(owner.address)
    expect(await shop.token()).to.be.properAddress
  })

  it("allows buy token", async function() {
    const tokenAmount = 3
    const txData = {
      value: tokenAmount,
      to: shop.address
    ...
    await expect(tx).to.emit(shop,"Bayer").withArgs(tokenAmount, buyer.address)
  })

  it("allowse to sell", async function() {
